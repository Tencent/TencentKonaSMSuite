...
abstract class PBES2Parameters extends AlgorithmParametersSpi {
    ...
    private String parseKDF(DerValue keyDerivationFunc) throws IOException {
        ...
        // keyLength INTEGER (1..MAX) OPTIONAL,
        Optional<DerValue> ksDer = pBKDF2_params.data.getOptional(DerValue.tag_Integer);
        if (ksDer.isPresent()) {
        ...
        // prf AlgorithmIdentifier {{PBKDF2-PRFs}} DEFAULT algid-hmacWithSHA1
        String kdfAlgo;
        Optional<DerValue> prfDer = pBKDF2_params.data.getOptional(DerValue.tag_Sequence);
        if (prfDer.isPresent()) {
            DerValue prf = prfDer.get();
            kdfAlgo_OID = prf.data.getOID();
            KnownOIDs o = KnownOIDs.findMatch(kdfAlgo_OID.toString());
            if (o == null || (!o.stdName().equals("HmacSM3")&&
                    !o.stdName().equals("HmacSHA1") &&
    ...
    private String parseES(DerValue encryptionScheme) throws IOException {
        ...
        if (sm4128CBC_OID.equals(cipherAlgo_OID)) {
            ...
            keysize = 128;
        } else if (aes128CBC_OID.equals(cipherAlgo_OID)) {
            cipherAlgo = "AES_128";
            // parameter is AES-IV 'OCTET STRING (SIZE(16))'
            cipherParam =
                new IvParameterSpec(encryptionScheme.data.getOctetString());
