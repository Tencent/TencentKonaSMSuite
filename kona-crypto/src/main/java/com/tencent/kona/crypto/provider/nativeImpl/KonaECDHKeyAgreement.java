 ...
 * @since   1.7
 */
public final class KonaECDHKeyAgreement extends KeyAgreementSpi {

    private static final Sweeper SWEEPER = Sweeper.instance();
    private NativeECDHKeyAgreement ka;
    ...
    private void init(Key key) throws
        ...
        int curveNID = Constants.getCurveNID(curve.getName());

        privateKey = ecPrivateKey;

        ECParameterSpec params = ecPrivateKey.getParams();
        byte[] priKeyValue = ECUtil.sArray(ecPrivateKey.getS(), params);
        ArrayUtil.reverse(priKeyValue);
        ka = new NativeECDHKeyAgreement(curveNID, priKeyValue);
        SWEEPER.register(this, new SweepNativeRef(ka));
    }
    ...
    private byte[] deriveKeyImpl(ECPrivateKey priv, ECPublicKey pubKey)
            throws InvalidKeyException {
        byte[] pubKeyValue = ECUtil.encodePoint(
                pubKey.getW(), pubKey.getParams().getCurve());
        return ka.deriveKey(pubKeyValue);
    }
}
