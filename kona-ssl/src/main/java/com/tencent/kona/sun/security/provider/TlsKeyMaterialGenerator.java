...
import static com.tencent.kona.sun.security.provider.TlsPrfGenerator.*;
...
public final class TlsKeyMaterialGenerator extends KeyGeneratorSpi {
    ...
    private SecretKey engineGenerateKey0(byte[] masterSecret) throws GeneralSecurityException {
        ...
        if (protocolVersion >= 0x0303) {
            ...
            byte[] seed = TlsPrfGenerator.concat(serverRandom, clientRandom);
            keyBlock = TlsPrfGenerator.doTLS10PRF(masterSecret, TlsPrfGenerator.LABEL_KEY_EXPANSION, seed,
                        keyBlockLen, md5, sha);
        } else {
            // SSL
            md5 = CryptoInsts.getMessageDigest("MD5");
        ...
        try {
            if (isExportable == false) {
                ...
                if (protocolVersion >= 0x0302) {
                    ...
                    byte[] tmp = TlsPrfGenerator.doTLS10PRF(clientKeyBytes,
                            TlsPrfGenerator.LABEL_CLIENT_WRITE_KEY, seed, expandedKeyLength, md5, sha);
                    clientCipherKey = new SecretKeySpec(tmp, alg);
                    Arrays.fill(tmp, (byte) 0);

                    tmp = TlsPrfGenerator.doTLS10PRF(serverKeyBytes, TlsPrfGenerator.LABEL_SERVER_WRITE_KEY, seed,
                            expandedKeyLength, md5, sha);
                    serverCipherKey = new SecretKeySpec(tmp, alg);
                    Arrays.fill(tmp, (byte) 0);

                    if (ivLength != 0) {
                        byte[] block = TlsPrfGenerator.doTLS10PRF(null, TlsPrfGenerator.LABEL_IV_BLOCK, seed,
                                ivLength << 1, md5, sha);
