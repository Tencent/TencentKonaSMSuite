...
abstract class HandshakeContext implements ConnectionContext {
    ...
    static final boolean allowUnsafeRenegotiation =
            Utilities.getBooleanProperty(
                    "com.tencent.kona.ssl.allowUnsafeRenegotiation", false);
    ...
    static final boolean allowLegacyHelloMessages =
            Utilities.getBooleanProperty(
                    "com.tencent.kona.ssl.allowLegacyHelloMessages", true);
    ...
    private static List<ProtocolVersion> getActiveProtocols(
        ...
        ArrayList<ProtocolVersion> protocols = new ArrayList<>();
        int tlcpIndex = -1;
            ...
            for (CipherSuite suite : sslConfig.enabledCipherSuites) {
                if (suite.isAvailable() && suite.supports(protocol)) {
                    if (isActivatable(sslConfig, suite,
                        ...
                        if (protocol == ProtocolVersion.TLCP11) {
                            tlcpIndex = protocols.size() - 1;
                        }
                        found = true;
                        break;
                    }
        ...
        if (!protocols.isEmpty()) {
            ...
            resetTLCP(tlcpIndex, protocols);
        }

        return Collections.unmodifiableList(protocols);
    }

    // If TLCP is present and its original position is not index 0,
    // must reset it to the original position.
    private static void resetTLCP(int tlcpIndex,
            List<ProtocolVersion> protocols) {
        if (tlcpIndex > 0) {
            // The protocol must be TLCP
            ProtocolVersion tlcp = protocols.remove(0);
            protocols.add(tlcpIndex, tlcp);
        }
    }
