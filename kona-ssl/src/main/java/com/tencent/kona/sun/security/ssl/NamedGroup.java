...
enum NamedGroup {
    ...
    SM2P256V1(0x00F9, "curvesm2",
            NamedGroupSpec.NAMED_GROUP_ECDHE,
            ProtocolVersion.PROTOCOLS_OF_TLCP,
            CurveDB.lookup("curvesm2")),

    // SM2 curve used by TASSL for ECDHE key exchange
    TA_SM2CURVE(0x0000, "curvesm2",
            NamedGroupSpec.NAMED_GROUP_ECDHE,
            ProtocolVersion.PROTOCOLS_OF_TLCP,
            CurveDB.lookup("curvesm2")),
...
//            PredefinedDHParameterSpecs.ffdheParams.get(8192)),
    ...
    private NamedGroup(int id, String name,
        ...
        if (mediator) {
            try {
                algParams =
                        CryptoInsts.getAlgorithmParameters(namedGroupSpec.algorithm);
...
//                }
                algParams = null;
                try {
                    CryptoInsts.getKeyAgreement(name);
