...
final class StatusResponseManager {
    ...
    StatusResponseManager() {
        ...
        int cap = AccessController.doPrivileged(
                new GetIntegerAction("com.tencent.kona.ssl.stapling.cacheSize",
                    DEFAULT_CACHE_SIZE));
        cacheCapacity = cap > 0 ? cap : 0;

        @SuppressWarnings("removal")
        int life = AccessController.doPrivileged(
                new GetIntegerAction("com.tencent.kona.ssl.stapling.cacheLifetime",
                    DEFAULT_CACHE_LIFETIME));
        cacheLifetime = life > 0 ? life : 0;

        String uriStr = GetPropertyAction
                .privilegedGetProperty("com.tencent.kona.ssl.stapling.responderURI");
        URI tmpURI;
        try {
        ...
        respOverride = Utilities
                .privilegedGetBooleanProperty("com.tencent.kona.ssl.stapling.responderOverride");
        ignoreExtensions = Utilities
    ...
    class OCSPFetchCall implements Callable<StatusInfo> {
        ...
        public StatusInfo call() {
            ...
            try {
                ...
                // conditions occur:
                // (1) The com.tencent.kona.ssl.stapling.ignoreExtensions property is
                //     true, or
